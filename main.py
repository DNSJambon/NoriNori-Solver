...
#choix du mode:
print("\033c")
s= int(input("Entrez \n- 1 pour créer une grille\n- 2 Pour résoudre une grille existante (le fichier instance doit etre dans 'grilles')\n: "))
if s==1:
   ...
   nom = input("Nom du fichier grille : ")
   if nom.endswith(".txt"):
      nom = nom[:-4]
   n = int(input("Taille de la grille : "))
   taille = 550 / n
...
#-----------------mode resolution de grille existante-------------------
if s==2:
   #tracer le contours des zones:
   dimension,zones = id.fichier_instance_en_zones("grilles/"+nom+".txt")
   for i in range(dimension):
      for j in range(dimension):
         ...
         if (i-1,j) not in zones[zone]:
            canvas.create_line(x1, y1, x1+taille, y1, width=5, fill=bord, state='disabled')
         if (i,j-1) not in zones[zone]:
            canvas.create_line(x1, y1, x1, y1+taille, width=5, fill=bord, state='disabled')
            
   #montre la solution:
   solution = sat.solveur("grilles/"+nom)
